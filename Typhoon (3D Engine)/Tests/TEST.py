...
bone = Bone()
bone.setState(Vector(0, 10, 0), Vector(3, 3, 3))
#bone.body.orientation.setComponent(1,0.3,0.1,0.5)
#bone.body.setAcceleration(0,-2,0)
bone2 = Bone()
bone2.setState(Vector(2, 15, -0.5), Vector(1, 1, 1))
bone2.body.setAcceleration(0,-3,0)
...
def graphicsSetup():
        ...
        floor = box (pos=vector( d/2, -0.25, d/2), size=vector(d, 0.5, d),  color = color.blue)
...
def run():
        ...
        while(True):
            ...
            #print(bone.body.position)
            bone.display()
            bone2.display()
            #for contact in cData.contactArray:
                #print(bone.body.position.y,bone2.body.position.y,contact.contactPoint.y)
run()